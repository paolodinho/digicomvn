...
Config nguồn sự thật: `.Codex/context/brand-info.md` (đọc TRƯỚC, không hardcode).
Quy tắc viết: skill global `content-writer` + rule `content-professional`, `typography-dash`.
Tham khảo khung generic: `~/Codex-Workspace/_shared/skills/seo-content-pipeline/SKILL.md`.
...
- Nội dung cốt lõi là toplist (danh sách đếm được) -> H1/title PHẢI ghi rõ dạng toplist kèm số
  ...
  `.Codex/rules/do-dont.md` mục "Title/H1 phải khớp DẠNG nội dung".
...
- **Không có SSH key (phiên di động/cloud qua Codex web/mobile)** -> dùng REST API qua
  `tools/wp-rest-publish.py` (mục B). Credential: `.Codex/secrets/wp_app.json` (gitignored,
...
`~/Codex-Workspace/_backups/routines/<ngày>/content-pipeline/` + 1 dòng manifest.md
(rule routine-backup). Nếu đang ở môi trường không truy cập được thư mục Codex-Workspace
...
1. Mở GSC qua Chrome thật (Codex-in-chrome, tài khoản hieudx3107@gmail.com), property
