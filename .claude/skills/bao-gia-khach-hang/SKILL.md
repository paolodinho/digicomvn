...
| Vị trí lưu | `11-bao-gia-khach/_noibo/` (thư mục riêng, KHÔNG lẫn với file gửi khách) | `11-bao-gia-khach/` (thư mục hiện có) |
| Header/màu | Nền ĐỎ đậm (`#B91C1C`), dòng cảnh báo to: **"⚠ FILE NỘI BỘ - TUYỆT ĐỐI KHÔNG GỬI KHÁCH"** ở dòng 1 | Nền brand xanh/teal (`#1C2035` / `#0E8C7F`) như file khách hàng bình thường |
...
2. Dựng **file nội bộ trước** - đầy đủ cột: NCC, mã NCC, giá vốn (giá NCC báo), % chiết
   khấu ngầm nếu có, giá bán thật, % lãi. Lưu vào `11-bao-gia-khach/_noibo/`.
3. Từ file nội bộ, lọc bỏ cột NCC/giá vốn/công thức -> dựng **file gửi khách** theo
   ...
   theo lĩnh vực + mục lục điều hướng nếu số lượng lớn). Lưu vào `11-bao-gia-khach/`.
4. Khi trả lời/gửi file cho Hiếu: **luôn nói rõ file nào là file nào** kèm full path
   (theo rule global `output-file-path.md`), ví dụ:
   - "File nội bộ (có giá vốn, ĐỪNG gửi khách): `<path>`"
   - "File gửi khách (đã ẩn NCC/công thức): `<path>`"
...
- Không đặt 2 file trong cùng 1 thư mục không phân biệt - phải khác thư mục
  (`_noibo/` vs thư mục gốc) VÀ khác tên (tiền tố `NOIBO-`) VÀ khác màu để nhìn là
  biết ngay, tránh Hiếu bấm nhầm gửi file có giá vốn cho khách.
